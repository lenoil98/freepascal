   ...
   begin
     ...
     While not SharedLibFiles.Empty do
      begin
        ...
        if ((((((((((s<>'c') and (s<>'iconv')) and (s<>'iconv')) and (s<>'iconv')) and (s<>'iconv')) and (s<>'iconv')) and (s<>'iconv')) and (s<>'iconv')) and (s<>'iconv')) and (s<>'iconv')) or reorder then
         begin
...
{$ifdef powerpc}
  if (target_info.abi=abi_powerpc_elfv2) and
    ...
    targetstr:='-b elf32-powerpcle -m elf32lppc'
  else
    targetstr:='-b elf32-powerpc -m elf32ppc';
{$endif powerpc}
{$ifdef powerpc64}
  if (target_info.abi=abi_powerpc_elfv2) and
     (target_info.endian=endian_little) then
    targetstr:='-b elf64-powerpcle -m elf64lppc'
  else
    targetstr:='-b elf64-powerpc -m elf64ppc';
{$endif powerpc64}
...
{$ifdef powerpc}
  ...
  RegisterImport(system_powerpc_freebsd,timportlibbsd);
  RegisterExport(system_powerpc_freebsd,texportlibbsd);
  RegisterTarget(system_powerpc_freebsd_info);
